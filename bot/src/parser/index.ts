...
/**
 * Shared pipeline: regex → AI classify → AI extract → save → match.
 * Called by both bot message handler and web scraper.
 */
export async function processMessageText(
  ...
  // Dedup check
  const dedupKey = `${sourceId}:${messageId}`;
  ...
  const job = await createParsedJob({
    ...
    poster_telegram_id: posterTelegramId ?? undefined,
    poster_username: posterUsername ?? undefined,
  });

  console.log(`[parser] New job parsed: "${job.title}" from source ${sourceTitle || sourceId} (${job.required_skills?.join(", ")})`);

  // Stage 5: DM poster about parsed job (throttled)
  if (posterTelegramId && bot) {
    const lastDm = posterDmThrottle.get(posterTelegramId) ?? 0;
    if (Date.now() - lastDm >= POSTER_DM_INTERVAL) {
      try {
        const miniAppUrl = process.env.MINI_APP_URL || "https://localhost:5173";
        await bot.api.sendMessage(posterTelegramId,
          `Мы нашли ваш заказ «${job.title}» в группе ${sourceTitle || sourceId}.\n\n` +
          `Верифицированные исполнители уже могут откликнуться. ` +
          `Откройте профиль, чтобы увидеть отклики:\n${miniAppUrl}/profile`,
        );
        posterDmThrottle.set(posterTelegramId, Date.now());
      } catch {
...
/**
 * Bot message handler — extracts context and delegates to shared pipeline.
 */
